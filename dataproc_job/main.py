...
# See https://stackoverflow.com/questions/43984068/does-spark-sql-autobroadcastjointhreshold-work-for-joins-using-datasets-join-op
# To disable Broadcasting large task binary with size USE 'spark.sql.autoBroadcastJoinThreshold' -1 in config
...
def write_parquet_file(dataframe_obj, Portfolio_Code):
...
if __name__ == "__main__":
    ...
    for file in complete_file_name:
        file = f'file://' + file    # to fix path error on Dataproc (default to hdfs://)
        ...
        # Sanity check Number of dataframe partition before writing to parquet
        ...
        merged_dataframe.repartition(100)
        
        # for dataframe in merged_data:
        write_parquet_file(merged_dataframe,Portfolio_Code)
    
    print('All file are complete transform and parqutize and upload to GCS bucket')
